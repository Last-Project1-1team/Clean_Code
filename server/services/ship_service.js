...
const findCustCodeNo = async (custno) => {
    let inordlist = await mariadb.query("shipmodel", [custno]).catch((err) => console.log(err));
    console.log(inordlist);
    return inordlist;
};
//lot단건조회
const findLotNo = async (lotno) => {
    let lotlist = await mariadb.query("lotnoscan", [lotno]).catch((err) => console.log(err));
    console.log(lotlist);
    return lotlist;
};
...
const findmodel = async (modelno) => {
    ...
    const modellist = await mariadb.query(modelsql, modelno).catch((err) => console.log(err));
    return modellist;
};
//
//
...
const addNewShip = async (ships) => {
    ...
    try {
        ...
        const lastList = await conn.query(sqlList.selectLastShipNo, [`OI${datePart}%`]);
        ...
        // 마스터 등록
        let insertMaster = [shipNo, ships[0].custcode, shipDate, ships[0].inordno, createdBy];
