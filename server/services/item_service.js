...
const findOutOrder = async (fromDate, toDate) => {
  let fromDate_form = formatFullDate(fromDate);
  let toDate_form = formatFullDate(toDate);
  let list = await mariadb.query("outordSelect", [fromDate, toDate]).catch((err) => console.log(err));
  return list;
};

const findInput = async (itemCode, itemName, fromDate, toDate) => {
  let fromDate_form = formatFullDate(fromDate);
  let toDate_form = formatFullDate(toDate);
  let list = await mariadb.query("selectItemInput", [`%${itemCode || ""}%`, `%${itemName || ""}%`, fromDate, toDate]).catch((err) => console.log(err));
  return list;
};
...
const finditemOutput = async (itemCode, itemName, outputDatefr, outputDateto) => {
  let list = await mariadb.query("selectOutputList", [`%${itemCode || ""}%`, `%${itemName || ""}%`, outputDatefr, outputDateto]).catch((err) => console.log(err));
  return list;
};
...
const addNewInsp = async (inputNo, result, items) => {
  let conn;
  ...
  try {
    ...
    const [last] = await conn.query(sqlList.selectLastInspNo, [`II${datePart}%`]);
    console.log(last);
    let seq = 1;
    if (last && last.item_insp_no) {
      const lastSeq = parseInt(last.item_insp_no.slice(-5));
      seq = lastSeq + 1;
    }
    ...
    console.log("프로시저 호출 시작");
    console.log(payload);
    await conn.query(` CALL SP_ITEM_INSP(?, @P_RETURN_CODE, @P_RETURN_MSG)  `, [JSON.stringify(payload)]);
    console.log("프로시저 정상 작동");
