...
router.get("/itemOutput", async (req, res) => {
    ...
    let itemCode = req.query.itemCode;
    let itemName = req.query.itemName;
    let outputList = await itemService
        .finditemOutput(itemCode, itemName, outputDatefr, outputDateto)
...
router.get("/prodLotList", async (req, res) => {
    ...
    let itemList = await itemService
        .findprodLotList(status, modelCode, modelRevision, modelName)
        .catch((err) => console.log(err));
    res.send(itemList);
});
