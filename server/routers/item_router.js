...
router.get("/itemOutput", async (req, res) => {
  console.log(req.query.outputDate);
  let outputDate = req.query.outputDate;
  let outputList = await itemService.finditemOutput(outputDate).catch((err) => console.log(err));
  res.send(outputList);
});
